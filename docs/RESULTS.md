...
Every number in the manuscript is traced here to the file it came from. `11n` is stock YOLO11n, `11-DLA` is YOLO11-DLA-n; `DLA` is strict placement with fallback disabled, `DLA+GPU` is the stock model built with `--allowGPUFallback`. All measurements: Jetson AGX Orin, L4T 36.4.4, TensorRT 10.3.0, batch one, 640×640, 50 W, `jetson_clocks` disabled.

The manuscript tables are **generated**, not typed:

```bash
make -C tools/paper precision-results    # writes tools/paper/tables/*.tex and figures/precision_latency.{pdf,svg}
```

That script reads the eight CSVs listed below and records their SHA-256 in [`../tools/paper/analysis/precision_results.json`](../tools/paper/analysis/precision_results.json), which is tracked and is the machine-readable version of this page. The CSVs themselves are campaign output under the untracked `logs/`: the recorded hashes are what ties a table cell to the run that produced it.

## Table I — Engine-plus-transfer latency
...
- FP16 rows: [`../tools/paper/analysis/engine_summary.csv`](../tools/paper/analysis/engine_summary.csv), derived by `run.py audit` from the four raw traces in `logs/fp16_matrix/*.times.json` (8,651 + 9,519 + 878 + 1,119 = **20,167 timing samples**, one launch per configuration). Mirrored with uniform names under `logs/icra2027_final/existing/micro/` and summarized in `logs/icra2027_final/existing/summary-micro/results.csv`.
...
## Table II — Accuracy on the 4,500 held-out COCO val2017 images
...
## Table III — Application pipeline
...
## Table IV — Module energy at 10 Hz
...
## Placement, Figure 1 and Figure 3

The placement claim and its inspector evidence are tabulated in [`BUILD.md`](BUILD.md). The FP16 layer profile behind `tools/paper/figures/fallback_profile.pdf` — 93.97% DLA, 4.73% reformats, 1.30% GPU compute, 34.312 ms summed device profile with 32.244 ms in DLA segments — comes from `logs/fp16_matrix/*.profile.json` via `run.py audit`, tabulated in [`../tools/paper/analysis/layer_profile.csv`](../tools/paper/analysis/layer_profile.csv) and recorded in [`../tools/paper/analysis/fp16_audit.json`](../tools/paper/analysis/fp16_audit.json). It characterizes FP16 only.

`figures/precision_latency.pdf` is generated by `make -C tools/paper precision-results`. `figures/teaser.pdf`, `figures/architecture_overview.pdf` and `figures/c2dla_detail.pdf` are hand-drawn vector schematics.

## Export, layout and decoder checks

| Claim | Value | Source |
|---|---|---|
| CPU external-decoder error, one random input (seed 7) | max abs 1.22×10⁻⁴ | `tools/paper/analysis/model_verification.json` |
